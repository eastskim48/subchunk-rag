...
def parse_global_top_r(raw_value: str) -> float:
...
class _DenseEmbeddingSelector(Compressor):
    def __init__(self):
        super().__init__()
...
class _GlobalDenseSelector(_DenseEmbeddingSelector):
    def __init__(self):
        super().__init__()
    ...
    def _deduplicate_sentence_refs(
        ...
        dedup_refs, dedup_scores = (
            _GlobalDenseSelector._deduplicate_sentence_refs_multi_scores(
                chunks=chunks,
...
class DenseSummarizer(_GlobalDenseSelector):
    def __init__(self):
        super().__init__()
        self.compare_embed_dir = os.getenv("COMPARE_EMBED_DIR")
        if not self.compare_embed_dir:
            raise ValueError(
                "COMPARE_EMBED_DIR must be set for compress_method='dense'"
            )
